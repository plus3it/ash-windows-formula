    ...
    while index < len(src):
        ...
        if src[index].upper() in REG_HIVES:
            ...
            try:
                ...
                policy["key"] = "\\".join([src[index], src[index + 1], src[index + 2]])
                ...
                raise SystemError(
                    "Whoops. Malformed policy in src_file? "
                    "Error at lines #{0}-{1}. Exception: {2}".format(
                        index + 1, index + 4
                    ),
                    exc,
                )
            ...
            raise SystemError(
                'Policy must begin with a "Configuration" line '
                'of "User" or "Computer". Received "{0}" at '
                "line #{1}".format(src[index], index + 1)
            )
    return policies
    ...
    for index, line in enumerate(src):
        ...
        if "\\" in line and line.startswith("MACHINE"):
            ...
            if not policy["vtype"].upper() in REG_TYPES:
                print(
                    "Line #{0}, registry type not supported by apply_lgpo_delta: {1}".format(
                        index + 1, line
                    )
                )
                continue
